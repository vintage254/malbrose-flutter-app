...
class CustomerReportService {
  ...
  Future<void> printCustomerReport(CustomerReport report, Customer customer) async {
    ...
    if (currentUser != null) {
      await DatabaseService.instance.logActivity({
        ...
        'action': DatabaseService.actionPrintCustomerReport,
        'details': 'Printed report #${report.reportNumber} for ${report.customerName}',
        'timestamp': DateTime.now().toIso8601String(),
        'username': currentUser.username,
      });
    }
    try {
      if (report.completedItems == null && report.pendingItems == null) {
        throw Exception('No items found in report');
      }
          ...
          children: [
            ...
            pw.Column(
              ...
              children: [
                ...
                pw.Text('CustomerReport #: ${report.reportNumber}'),
                pw.Text('Date: ${DateFormat('MMM dd, yyyy').format(report.createdAt)}'),
                if (report.dueDate != null)
                  pw.Text('Due Date: ${DateFormat('MMM dd, yyyy').format(report.dueDate!)}'),
              ],
            ),
          ],
        ),
      ...
      if ((report.completedItems != null && report.completedItems!.isNotEmpty) ||
          (report.pendingItems != null && report.pendingItems!.isNotEmpty)) {
        ...
        if (report.completedItems != null && report.completedItems!.isNotEmpty) {
          children.addAll([
            ...
            pw.Table.fromTextArray(
              ...
              data: report.completedItems!.map((item) => [
        ...
        if (report.pendingItems != null && report.pendingItems!.isNotEmpty) {
          children.addAll([
            ...
            pw.Table.fromTextArray(
              headers: ['Order #', 'Product', 'Quantity', 'Unit Price', 'Total'],
              data: report.pendingItems!.map((item) => [
                item.orderId.toString(),
                item.productName,
                '${item.quantity} ${item.isSubUnit ? (item.subUnitName ?? "pieces") : "units"}',
                'KSH ${item.sellingPrice.toStringAsFixed(2)}',
              ...
              children: [
                        ...
                        children: [
                          ...
                          pw.Row(
                            children: [
                              ...
                              pw.Text(
                                DateFormat('MMM dd, yyyy').format(report.dueDate!),
                                style: pw.TextStyle(
                                  font: boldFont,
                                  fontSize: 12,
                                  color: PdfColors.blue900,
                ...
                pw.Table(
                  ...
                  children: [
                    ...
                    // Table Items
                    if (report.completedItems != null)
                      ...report.completedItems!.map((item) => pw.TableRow(
                        children: [
                    ...
                    if (report.pendingItems != null)
                      ...report.pendingItems!.map((item) => pw.TableRow(
                        children: [
                          _buildTableCell(item.productName, font),
                          _buildTableCell(
                            item.quantity.toString() + 
  ...
  Future<CustomerReport> generateCustomerReport(int customerId) async {
    ...
    return await db.transaction((txn) async {
      ...
      // Generate a proper report number
      final reportNumber = await generateCustomerReportNumber();
      ...
      // Save the report
      final reportId = await txn.insert(DatabaseService.tableCustomerReports, report.toMap());
      ...
      if (currentUser != null) {
        await DatabaseService.instance.logActivity({
          ...
          'details': 'Generated report #${report.reportNumber} for ${report.customerName}',
          'timestamp': DateTime.now().toIso8601String(),
          'username': currentUser.username,
        });
      }
  ...
  Future<String> generateCustomerReportNumber() async {
    final now = DateTime.now();
    final prefix = 'REP';
    final date = DateFormat('yyyyMMdd').format(now);
    
    // Get the last report number for today
    final db = await DatabaseService.instance.database;
    final result = await db.rawQuery('''
      SELECT report_number 
      FROM ${DatabaseService.tableCustomerReports} 
